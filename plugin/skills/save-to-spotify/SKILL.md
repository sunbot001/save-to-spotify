...
4. **Produce** — Generate audio:
   - **If using Sun**: Send the full script to Sun's `/api/audio/generate` endpoint (see [references/audio-providers.md](references/audio-providers.md) § Sun). Genesis handles LLM rewriting, multi-speaker dialogue, TTS, audio mixing, intro/outro music, and loudness normalization. Poll until lectures are GENERATED, download the lecture MP3s from `sb.sunapp.ai/storage/v1/object/public/lectures/generated_lectures/{lecture_id}.mp3`, concatenate into a single episode, then build `timeline.json` from lecture titles and durations.
